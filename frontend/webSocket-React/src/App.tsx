...
function App() {
  ...
  // Guarda la conexión WebSocket actual.
  // Comienza en null porque todavía no estamos conectados.
  const [socket, setSocket] = useState<WebSocket | null>(null);

  // Array con todos los mensajes que se muestran en el chat.
  const [messages, setMessages] = useState<ChatMessage[]>([]);

  // Array con los nombres de los usuarios actualmente conectados.
  const [users, setUsers] = useState<string[]>([]);

  // Contiene el texto que el usuario está escribiendo en el input del mensaje.
  const [input, setInput] = useState("");

  // Contiene el nombre que el usuario ingresó al entrar al chat.
  const [username, setUsername] = useState("");

  // Indica si el usuario ya ingresó al chat.
  // false = pantalla de ingreso
  // true  = pantalla del chat
  ...
  useEffect(() => {
    ...
    // envía un mensaje al cliente, es decir, cuando se ejecuta => client.send() esto se ejecuta
    ws.onmessage = (event) => {
